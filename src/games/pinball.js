...
function tick(s,input){
  ...
  if(!s.lane){
    ...
    segment(s,105,545,185,570-65*s.left,9,input.left?(s.left>oldLeft?820:580):0);
    segment(s,295,545,215,570-65*s.right,9,input.right?(s.right>oldRight?820:580):0);
...
export function createPinballGame(env,state){
 ...
 const data=()=>JSON.parse(JSON.stringify(s));const save=(force=false)=>env.save(data(),force);const active=()=>!destroyed&&!finished&&!doc.hidden&&!env.isPaused()&&env.isActive();
 ...
 function draw(){
  ...
  [[105,185,s.left],[295,215,s.right]].forEach(([x,tip,up])=>{path([[x+2,552],[tip+2,577-65*up]],'#000b',22);path([[x,545],[tip,570-65*up]],'#ac692f',20);path([[x,542],[tip,567-65*up]],'#ffd278',13);circle(x,543,6,'#b5c9cc','#445661');});
