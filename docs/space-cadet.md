...
静态美术由 imagegen 按原几何模板重绘；动态球、挡板、靶和灯仍由引擎绘制。默认素材不是 Microsoft 原版贴图。想使用本机原版素材时，在游戏右下角「素材」选择一份 PINBALL.DAT／CADET.DAT，可一并选择 sound*.wav，然后重开。素材保存在当前浏览器，不上传服务器。
