...
3.10 adds an optional `Module.cadetPresent(pixels, stride, background, backgroundStride, width, height, offsetX, offsetY)` callback after native rendering. Pointers are byte offsets into `HEAPU32`; strides count pixels. Both images are ARGB8888 (BGRA bytes on this little-endian WASM target). Read the table bitmap from `loader::query_visual("table")`, not the legacy score-sidebar background. The native framebuffer has already applied depth tests and has not yet applied the nudge offset. The host applies that offset once to both display layers.

`assets/space-cadet/display.js` caches the 1105×1423 source artwork at the visible canvas device resolution (up to 3× DPR / 2.2 megapixels). Pixels equal to the native table background become transparent; all differing native pixels are composited above it. This preserves native balls, flippers, lamps and occlusion while avoiding a 365px downsample of the static artwork. No new physics or display loop runs in the host: the engine callback drives each frame, and a resize/quality change can repaint the frozen framebuffer. Classic mode and imported DAT data display the opaque native framebuffer. Dynamic sprites retain the original resolution; this is not a new high-resolution 3D mesh engine.
